...
#include <string.h>
#include <ctype.h>
#include <stdlib.h> //imports malloc
...
int main() {
	...
	char* initialArray = (char*)malloc(26 * sizeof(char));
	printf("Initial Array Memory Location: %p\n", &initialArray);
	char* replacementArray = (char*)malloc(26 * sizeof(char));
	...
	free(initialArray); //frees up the memory when the program is finished
	free(replacementArray);
}